//! TLS encryption tunnel via ghostunnel proxy.
//!
...
pub struct TLSChunnel<A = TlsConnAddr> {
    ...
    listen_cn: Arc<StdMutex<Option<Result<(UnixListener, process::GhostTunnel), Report>>>>,
    send_cn: Arc<StdMutex<Option<Result<(UnixStream, process::GhostTunnel), Report>>>>,
    ...
    fn picked<'s>(&mut self, _nonce: &'s [u8]) -> Pin<Box<dyn Future<Output = ()> + Send + 's>> {
        ...
        let listen = self.listen.map(|srv| {
            ...
            (
                ...
                async move {
                    let mut gt = process::GhostTunnel::start_server(
                        srv,
                        &srv_unix,
                        &binary_path,
                        &certs_location,
        ...
        let send = self.remote.map(|rem| {
            ...
            (
                send_cn,
                async move {
                    let mut gt = process::GhostTunnel::start_client(
                        rem,
                        &cli_unix,
                        &binary_path,
                        &certs_location,
...
pub struct TlsConn<A> {
    ...
    _tls_tunnel_server_handle: Option<process::GhostTunnel>,
    _tls_tunnel_client_handle: Option<process::GhostTunnel>,
    _phantom: std::marker::PhantomData<A>,
}
...
impl<A> TlsConn<A> {
    fn new(
        remote_addr: Option<SocketAddr>,
        listen: Option<(UnixListener, process::GhostTunnel)>,
        send: Option<(UnixStream, process::GhostTunnel)>,
    ) -> Self {
        match (listen, send) {
