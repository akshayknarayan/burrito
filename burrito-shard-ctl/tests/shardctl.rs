...
fn recursive_shardctl_test() -> Result<(), Error> {
    ...
    rt.block_on(async move {
        ...
        let shardctl = burrito_shard_ctl::ShardCtl::new(&redis_addr, Some(root.clone())).await?;
        ...
        use burrito_shard_ctl::{proto, Shard, TreeShardInfo};
        assert!(matches!(
            si,
            Shard::Sharded(TreeShardInfo {
        ...
        let l1 = match si {
            Shard::Sharded(TreeShardInfo { shard_addrs, .. }) => shard_addrs,
            _ => unreachable!(),
        };

        debug!(&log, "Checking shard addrs"; "shard_addrs" => ?&l1);

        assert!(matches!(
            &l1[..],
            [
                Shard::Sharded(TreeShardInfo {
                    canonical_addr: proto::Addr::Udp(s1),
                    shard_info: proto::SimpleShardPolicy {
                        packet_data_offset: 18,
                        packet_data_length: 4,
                    },
                ...
                Shard::Sharded(TreeShardInfo {
                    canonical_addr: proto::Addr::Udp(s2),
                    shard_info: proto::SimpleShardPolicy {
                        packet_data_offset: 18,
                        packet_data_length: 4,
                    },
            ...
            ] if s1.ip().is_loopback() && s1.port() == 59423 && s2.ip().is_loopback() && s2.port() == 59424
        ));

        let s1s = match l1[0] {
            Shard::Sharded(TreeShardInfo {
                ref shard_addrs, ..
            }) => shard_addrs.as_slice(),
            _ => unreachable!(),
        };
        ...
        assert!(matches!(
            s1s,
            [
                Shard::Addr(proto::Addr::Udp(s1)),
                Shard::Addr(proto::Addr::Udp(s2)),
            ] if s1.ip().is_loopback() && s1.port() == 59427 && s2.ip().is_loopback() && s2.port() == 59428
        ));
        ...
        assert!(matches!(
            ...
            [
                ...
                Shard::Addr(proto::Addr::Udp(s3)),
            ] if 
                s1.ip().is_loopback() && s1.port() == 59425 
                && s2.ip().is_loopback() && s2.port() == 59426 
                && s3.ip().is_loopback() && s3.port() == 59429
        ));
        ...
        assert!(matches!(
            ...
            [
                Shard::Addr(proto::Addr::Udp(s1)),
                Shard::Addr(proto::Addr::Udp(s2)),
            ] if s1.ip().is_loopback() && s1.port() == 59430 && s2.ip().is_loopback() && s2.port() == 59431
        ));
        ...
        assert!(matches!(
            ...
            [
                Shard::Addr(proto::Addr::Udp(s1)),
                Shard::Addr(proto::Addr::Udp(s2)),
            ] if s1.ip().is_loopback() && s1.port() == 59432 && s2.ip().is_loopback() && s2.port() == 59433
        ));
...
async fn start_rec_server(root: &std::path::Path, start: u16) {
...

    {
        ...
        for (name, address) in ns.iter().chain(ps.iter()).enumerate().map(|(i, a)| {
            (
                a,
                format!("udp://127.0.0.1:{}", start + i as u16)
                    .parse()
                    .unwrap(),
            )
        }) {
...
fn shardctl_test() -> Result<(), Error> {
    ...
    rt.block_on(async move {
        ...
        for (p, r) in sa
            .iter()
            .map(|a| match a {
                Shard::Addr(proto::Addr::Udp(sa)) if sa.ip().is_loopback() => sa.port(),
...
fn basic_shardctl_test() -> Result<(), Error> {
    ...
    rt.block_on(async move {
        ...
        let shardctl = burrito_shard_ctl::ShardCtl::new(&redis_addr, None)
            .await
