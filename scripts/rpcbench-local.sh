...
sudo docker container wait lrpcclient
echo "-> rpcbench-client done"
sudo docker logs lrpcclient > $out/work_sqrts_1000-iters_10000_periter_3_fulldocker-burrito_localhost_docker.log
sudo docker cp lrpcclient:/app/res.data $out/work_sqrts_1000-iters_10000_periter_3_fulldocker-burrito_localhost_docker.data
sudo docker cp lrpcclient:/app/res.trace $out/work_sqrts_1000-iters_10000_periter_3_fulldocker-burrito_localhost_docker.trace
...
sudo RUST_LOG=info,burrito_discovery_ctl=debug ./target/release/burrito-discovery-ctl \
    ...
    --net-addr=$docker_host_addr \
    -f \
    > $out/sec1-burritoctl-discovery.log 2> $out/sec1-burritoctl-discovery.log &
burritoctl=$!
sleep 2
echo "--> start burrito-localname-ctl"
sudo RUST_LOG=info,burrito_localname_ctl=debug ./target/release/burrito-localname \
    ...
    -o /var/run/burrito-docker.sock \
    -f \
    > $out/sec1-burritoctl-local.log 2> $out/sec1-burritoctl-local.log &
lburritoctl=$!
sleep 2
sudo docker run --name rpcbench-server --security-opt seccomp=unconfined -e RUST_LOG=debug -d $image_name ./pingserver \
    --burrito-addr="rpcbench" \
    --burrito-root="/burrito" \
    --port="4242"
sleep 2
echo "-> start rpcbench-client"
sudo docker run --name lrpcclient --security-opt seccomp=unconfined -e RUST_LOG=debug,rpcbench=trace,burrito_addr=trace -t -d $image_name ./pingclient \
    --addr="rpcbench" \
    --burrito-root="/burrito" \
    --amount 1000 -w 4 -i 10000 \
    --reqs-per-iter 3 \
...
sudo docker container wait lrpcclient
echo "-> rpcbench-client done"
sudo docker logs lrpcclient > $out/work_sqrts_1000-iters_10000_periter_3_noseccomp-burrito_localhost_docker.log
sudo docker cp lrpcclient:/app/res.data $out/work_sqrts_1000-iters_10000_periter_3_noseccomp-burrito_localhost_docker.data
sudo docker cp lrpcclient:/app/res.trace $out/work_sqrts_1000-iters_10000_periter_3_noseccomp-burrito_localhost_docker.trace
...
echo "-> rpcbench-client done"
sudo docker logs lrpcclient > $out/work_sqrts_1000-iters_10000_periter_3_fulldocker-burrito_localhost_docker.log
sudo docker cp lrpcclient:/app/res.data $out/work_10ksqrts_1000-iters_10000_periter_3_fulldocker-burrito_localhost_docker.data
sudo docker cp lrpcclient:/app/res.trace $out/work_10ksqrts_1000-iters_10000_periter_3_fulldocker-burrito_localhost_docker.trace
...
sudo docker container wait lrpcclient
echo "-> rpcbench-client done"
sudo docker logs lrpcclient > $out/work_10ksqrts_1000-iters_10000_periter_3_noseccomp-burrito_localhost_docker.log
sudo docker cp lrpcclient:/app/res.data $out/work_10ksqrts_1000-iters_10000_periter_3_noseccomp-burrito_localhost_docker.data
sudo docker cp lrpcclient:/app/res.trace $out/work_10ksqrts_1000-iters_10000_periter_3_noseccomp-burrito_localhost_docker.trace
