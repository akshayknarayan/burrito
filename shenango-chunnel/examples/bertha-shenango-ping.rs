...
fn main() -> Result<()> {
    ...
    rt.block_on(async move {
        let sk = ShenangoUdpSkChunnel::new(&cfg);
        match client {
            Some(ip) => {
...
async fn do_server(mut sk: ShenangoUdpSkChunnel, port: u16) -> Result<()> {
...
#[tracing::instrument(err)]
async fn do_client(mut sk: ShenangoUdpSkChunnel, remote: SocketAddrV4) -> Result<Vec<Duration>> {
