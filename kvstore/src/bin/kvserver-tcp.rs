...
fn sk_shard_addrs(
    ...
    (0..num_shards + 1)
        .map(|_| std::net::SocketAddr::new(ip_addr, base_port)) // all bound to same port
        .collect()
}
...
async fn main() -> Result<(), StdError> {
    ...
    let si = ShardInfo {
        canonical_addr: proto::Addr::Tcp(addrs[0]),
        shard_addrs: if opt.no_shard_ctl {
            vec![]
        } else {
            addrs[1..].iter().map(|a| proto::Addr::Tcp(*a)).collect()
        },
        shard_info: proto::SimpleShardPolicy {
            packet_data_offset: 18, // TODO check! probably wrong with tcp
    ...
    let ls: Result<Vec<_>, _> = addrs
        .into_iter()
        .map(|addr| {
            //let x = std::net::TcpListener::bind(addr)?;
            //Ok::<_, StdError>(tokio::net::TcpListener::from_std(x)?)
            ...
            Ok::<_, StdError>(tokio::net::TcpListener::from_std(socket.into())?)
        })
        .collect();
    let ls = ls?;

    debug!(&log, "Starting"; "ls" => ?ls);
