...
use shenango_bertha::RawKvClient;
...
fn main() -> Result<(), Report> {
    ...
    shenango::runtime_init(
        ...
        move || {
            if !opt.skip_loads {
                let mut cl = RawKvClient::new(shard_addrs.clone());
            ...
            for (cid, ops) in group_by_client(accesses).into_iter() {
                let cl = RawKvClient::new(shard_addrs.clone());
...
fn exec_op(op: Op, cl: &RawKvClient) -> Result<Option<String>, Report> {
    match op {
        Op::Get(_, k) => cl.get(k),
...
fn do_requests(
    access_by_client: HashMap<usize, (RawKvClient, Vec<Op>)>,
    ...
    fn time_req(cl: &RawKvClient, op: Op) -> Result<Duration, Report> {
...
fn do_loads(cl: &mut RawKvClient, loads: Vec<Op>) -> Result<(), Report> {
    info!("starting loads");
